...
def focal_loss(s,sxing,ndims,gamma=1):
    st = torch.where(torch.ge(sxing, 0.01), s, 1 - s)
    factor = torch.pow(1. - st, gamma )
    #print('the factor is', factor.shape)
    unsummed_out = torch.mul((s-sxing), (s-sxing)) * factor #[1,20,300,400]
    out = reduce(lambda t, _: t.sum(-1, keepdim=False), range(ndims), unsummed_out)#[1,20]
    return out
def focal_reg_losses(heatmaps, mu_t, sigma_t,gamma=1):
    ndims = mu_t.size(-1)#2
    assert heatmaps.dim() == ndims + 2, 'expected heatmaps to be a {}D tensor'.format(ndims + 2)
    assert heatmaps.size()[:-ndims] == mu_t.size()[:-1]

    gauss = make_gauss(mu_t, heatmaps.size()[2:], sigma_t)
    divergences = focal_loss(heatmaps, gauss,ndims,gamma=1)
