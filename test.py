...
class WRDNet(nn.Module):
    ...
    def forward(self,x):
        # 1. Run the images through our PAViT
        embed = []
        ...
        for idx in range(self.num_stages):
            if idx == 0:
                x = F.interpolate(embed[idx], size=(50, 38), mode='bilinear', align_corners=True)
            if idx == 1:
                x = F.interpolate(x, size=(100, 75), mode='bilinear', align_corners=True)
            if idx == 2:
                x = F.interpolate(x, size=(200, 150), mode='bilinear', align_corners=True)
            if idx == 3:
                x = F.interpolate(x, size=(400, 300), mode='bilinear', align_corners=True)
...
class Model(nn.Module):
    def __init__(self,
                 ...
                 num_classes=1000,
                 in_channels=3,
                 out_channels=20,*args,**kwargs):

        super(Model,self).__init__()
        self._net = PAViT(num_stages=num_stages,
            num_path=num_path,
...
weight = []
for j in range(3):
    pweight='./models/'+str(j)+'_model_epoch100.pth'
    weight.append(pweight)

coords_path = './results/coordsresult/'
...
for j,(train_index,val_index) in enumerate(kf.split(data_lst)):
    ...
    with torch.no_grad():
        for img,target_var,name in val_loader:
            ...
            imgsize_tensor = torch.Tensor([300, 400]).to(device)
            ...
            name = ''.join(name)
            np.save(coords_path+f'val_{100}_{name}.npy',save_coords.cpu().numpy()[0])
    end = time.time()
