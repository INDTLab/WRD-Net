...
class Patch_Embed_stage(nn.Module):
    """Atrous Spatial Pyramid comprised of `DWCPatchEmbed` layers."""
    def __init__(self, embed_dim, num_path=4, isPool=False):
        ...
        self.patch_embeds = nn.ModuleList([
            DWCPatchEmbed(
                ...
                embed_dim=embed_dim,
                patch_size=3,
                stride=2 if isPool else 1,#and idx == 0 else 1,
                padding = 1 if idx == 0 else 2 if idx == 1 else 5,
                dilation = 1 if idx == 0 else 2 if idx == 1 else 5,
...
class MHCABlock(nn.Module):
    ...
    def forward(self, x, size):
        ...
        if self.cpe is not None:
            x = self.cpe(x, size)
        cur = self.norm1(x)
        z = self.drop_path(self.factoratt_crpe(cur, size))
        return z


class MHCAEncoder(nn.Module):
    """Parallel Attention Vision Transformer block comprised of `MHCA` blocks."""
    def __init__(
        self,
        dim,
        num_layers=1,
...
class MHCA_stage(nn.Module):
    ...
    def __init__(
        ...
        self.aggregate = Conv2d_BN(embed_dim * 2,
                                   out_embed_dim,
                                   act_layer=nn.Hardswish)
        self.mlp = Mlp(in_features=embed_dim, hidden_features=embed_dim * mlp_ratio)
        self.drop_path = DropPath(
            drop_path) if drop_path > 0.0 else nn.Identity()
    ...
    def forward(self, inputs):
        ...
        for x, encoder in zip(inputs, self.mhca_blks):
            # [B, C, H, W] -> [B, N, C]
            _, _, H, W = x.shape
            x_4 = x.flatten(2).transpose(1, 2)
            z = encoder(x_4, size=(H, W))
            # [B, C, H, W] -> [B, N, C]
            z = z.flatten(2).transpose(1, 2)
            y = y + z
        # [B, C, H, W] -> [B, N, C]
        i = inputs[0].flatten(2).transpose(1, 2)
        x = y + i
        cur = self.norm2(x)
